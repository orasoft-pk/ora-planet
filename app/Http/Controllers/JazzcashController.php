...
class JazzcashController extends Controller
{
    ...
    {
        // Set API key
        $this->merchant_id 	  = env('JAZZCASH_MERCHANT_ID');
		$this->password 	  = env('JAZZCASH_PASSWORD');
		$this->integrity_salt = env('JAZZCASH_INTEGERITY_SALT');
		$this->currency 	  = env('JAZZCASH_CURRENCY_CODE');
		$this->language 	  = env('JAZZCASH_LANGUAGE');
		$this->post_url 	  = env('JAZZCASH_HTTP_POST_URL');
    } 
