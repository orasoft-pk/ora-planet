                                    ...
                                    <div class="table-responsive">
                                      <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                              ...
                                              @foreach($prods as $prod)    
                                                      ...
                                                      {{--  @php
                                                      $product = App\Models\Product::findOrFail($prod->user_id);
                                                      @endphp  --}}
                                                      @if(isset($prod->user->shop_name))
                                                      <td><a href="{{route('front.vendor',str_replace(' ', '-',($prod->user->shop_name)))}}">{{$prod->user->shop_name}}</a></td>
                                                      @else
                                                      <td></td>
                                                      @endif
                                            ...
                                            <button class="btn btn-{{$prod->status == 1 ? "success" : "danger"}} product-btn dropdown-toggle btn-xs" type="button" data-toggle="dropdown" style="font-size: 14px;">{{$prod->status == 1 ? "Activated" : "Deactivated"}}
                                                <span class="caret"></span></button>
                                                      ...
                                                      <td>
                                                        ...
                                                        <a href="javascript:;" data-href="{{route('admin-prod-delete',$prod->id)}}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger product-btn"><i class="fa fa-trash"></i>Remove</a>
                                                      </td>
                                                  </tr>
                                              @endforeach
                                                </tbody>
        ...
        $("#feature .modal-content").html('');
            $.ajax({
                    ...
                    success:function(data){
                      ...
                      data[6] = data[6] == 1 ? "checked":"";
                        $("#feature .modal-content").append(''+
        '<form class="form-horizontal" action="{{url('/')}}/admin/product/feature/'+data[7]+'" method="POST">'+
        '{{csrf_field()}}'+
            '<div class="modal-header">'+
              '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'+
                '<h4 class="modal-title text-center" id="myModalLabel2">Product Title:'+data[8]+'</h4>'+
                '</div>'+
                '<div class="modal-body">'+
                                      ...
                                      '<div class="form-group">'+
                                        ...
                                        '<label for="check5">Add Product to {{$lang->latest_special}}</label>'+
                                            ...
                                            '<div class="form-group">'+
                                                ...
                                                '<div class="col-sm-9 col-sm-offset-3">'+
                                                  '<div class="btn btn-default checkbox1">'+
                                                    '<input type="checkbox" id="check6" name="deal_of_the_day" value="1" '+data[5]+'>'+
                                                '<label for="check6">Add Product to Deal of the DAY</label>'+
                                                  '</div>'+
                                                '</div>'+
                                            '</div>'+
                                            '<div class="form-group">'+
                                                ...
                                                '<div class="col-sm-9 col-sm-offset-3">'+
                                                  '<div class="btn btn-default checkbox1">'+
                                                    '<input type="checkbox" id="check7" name="festival" value="1" '+data[6]+'>'+
                                                '<label for="check7">Add Product to Fasitival</label>'+
                                                  '</div>'+
                                                '</div>'+
                                            '</div>'+
                '</div>'+
