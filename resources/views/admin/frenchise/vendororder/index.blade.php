                                      ...
                                      <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                              ...
                                              <tbody>
                                                @foreach($orders as $order)                                                  
                                                    ...
                                                    <td> <a href="{{route('admin-frenchise-order-invoice',$order->id)}}">{{sprintf("%'.08d", $order->id)}}</a></td>
                                                    <td> {{$order->qty}}</td>
                                                    <td> {{$order->currency_sign}}{{ round($order->price * $order->order->currency_value , 2) }}</td>
                                                    <td> {{$order->order->method}}</td>
                                                      <td>
                                                        <input type="hidden" value="{{$order->customer_email}}">
                                                        <a href="{{route('admin-frenchise-order-show',$order->id)}}" class="btn btn-primary product-btn"><i class="fa fa-check"></i> View Details</a>
                                            ...
                                            ">{{ucfirst($order->status)}}
                                                <span class="caret"></span></button>
                                                        <ul class="dropdown-menu">
                                                            <li><a href="javascript:;" data-href="{{route('admin-frenchise-order-status',['id' => $order->id, 'status' => 'processing'])}}" data-toggle="modal" data-target="#confirm-delete">Processing</a></li>
                                                            <li><a href="javascript:;" data-href="{{route('admin-frenchise-order-status',['id' => $order->id, 'status' => 'completed'])}}" data-toggle="modal" data-target="#confirm-delete">Completed</a></li>
                                                            <li><a href="javascript:;" data-href="{{route('admin-frenchise-order-status',['id' => $order->id, 'status' => 'declined'])}}" data-toggle="modal" data-target="#confirm-delete">Declined</a></li>
                                                        </ul>
                                                        </span>
                                                      </td>
                                                  </tr>
