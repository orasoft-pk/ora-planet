                                            ...
                                            <div class="table-responsive">
                                                <table id="product-table_wrapper" class="table table-striped table-hover products dt-responsive" cellspacing="0" width="100%">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td><a style=" color:{{$gs->colors == null ? '#337ab7':$gs->colors}};" href="{{route('front.vendor',str_replace(' ', '-',($withdraw->user->shop_name)))}}" target="_blank">{{$withdraw->user->shop_name}}</a></td>
                                                            ...
                                                            <td>
                                                                <a href="{{route('sub_head_office_vendor_wtd',$withdraw->id)}}" class="btn btn-primary product-btn"><i class="fa fa-eye"></i> View Details</a>
                                                                @if($withdraw->status == "pending")
                                                                <a href="javascript:;" data-href="{{route('sub_head_office_vendor_wt_accept',$withdraw->id)}}" data-toggle="modal" data-target="#confirm-delete" class="btn btn-success product-btn"><i class="fa fa-send"></i> Accept</a>
                                                                <a href="javascript:;" data-href="{{route('sub_head_office_vendor_wt_reject',$withdraw->id)}}" data-toggle="modal" data-target="#confirm-delete1" class="btn btn-danger product-btn"><i class="fa fa-trash"></i> Reject</a>
                                                                @endif
                                                            </td>
                                                        </tr>
                                                        @endforeach
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $(".add-button").append('<div class="col-sm-4 add-product-btn text-right">' +
            '<a href="{{route('sub_head_office_vendor_pending')}}" class="add-newProduct-btn">' +
